...
from src.CMUnet import CMU_Net
        ...
        for input, target, _ in val_loader:
            ...
            if config['deep_supervision']:
                ...
                for output in outputs:
                    loss += criterion(output, target)
                loss /= len(outputs)
                iou,dice,SE, PC, F1,SP,ACC = iou_score(outputs[-1], target)
            else:
                output = model(input)
                loss = criterion(output, target)
                iou,dice,SE, PC, F1,SP,ACC = iou_score(output, target)
...
def main():
    ...
    for epoch in range(config['epochs']):
        ...
        print('loss %.4f - iou %.4f - val_loss %.4f - val_iou %.4f - val_dice %.4f - val_SE %.4f - val_PC %.4f - val_F1 %.4f - val_SP %.4f - val_ACC %.4f'
              % (train_log['loss'], train_log['iou'], val_log['loss'], val_log['iou'],val_log['dice'],val_log['SE'],val_log['PC'],val_log['F1'],val_log['SP'],val_log['ACC']))
